		...
		from("direct:candidateInfo").log("AppliedCandidateInfo : ${body}").process(new Processor() {
			...
			public void process(Exchange exchange) throws Exception {
				...
				if (services.appliedCandidateInfoExists(appliedCandidateInfo)) {
					...
					exchange.getMessage().setHeader(Exchange.HTTP_RESPONSE_CODE, 201);
					String subject = services.selectCandidateInformation(appliedCandidateInfo.getStatus(), appliedCandidateInfo.getJobId(), appliedCandidateInfo.getRole());
...
//		------------------------23. After clicking on screening should be moved to Screening Before On-Boarding--------------------------
		...
		from("direct:selectedForScreeningRound").process(exchange -> {
			...
			String subject = services.selectCandidateInformation(candidateInfo.getStatus(), candidateInfo.getJobId(), candidateInfo.getRole());
...
//	--------------33. After clicking reject delete record and send mail------------------
		...
		from("direct:deleteCandidateInformation").process(exchange -> {
			String email = exchange.getIn().getHeader("email", String.class);
			AppliedCandidateInformation candidateInfo = services.findByEmail(email);
			String subject = services.deleteCandidateInformation(candidateInfo.getEmail(), candidateInfo.getStatus(), candidateInfo.getJobId(), candidateInfo.getRole());
